...
class PuzzleWorld:
    def __init__(self, C, parent = None, pos_puzzleWorld=[0.0, 0.3, 0.05], q_start = [-4*0.04, 0.0, 0.005], q_goal = [-4*0.04, 4*0.04, 0.005]):
        self.C = C
        self.parent = parent
    ...
    def build(self, withbox=False):
        ...
        if withbox == True:
            #self.C.addFrame("box", "puzzle_world") \
            self.C.addFrame("box", "table") \
                .setJoint(ry.JT.rigid) \
                .setShape(ry.ST.ssBox, [0.03, 0.03, 0.03, 0.0]) \
                .setPosition(self.q_start) \
                .setColor([1, 0, 0]) \
                .setContact(1) \
                .setMass(0.1)
...
# puzzle_world = PuzzleWorld(C, 'None', [0.0, 0.3, 0.65], [-0.12, .0, .65], [0.12, .12, .65])
# puzzle_world.build()
# C.view(True)
